...
def down_block(inputs, num_filters, kernel_dims=3):
	"""Creates the contracting path blocks used in the UNET.

	Keyword agurments:
	inputs -- Input tensor to this block.
	num_filters -- The number of filters to be used in this block's convolution layers.
	kernel_dims -- The kernel dimension used during this block's convolution layers (default 3). 

	Returns:
	x -- The processed tensor after passing through this block's layers.
	"""
...
def up_block(c_large, c_small, num_filters, strides_dims=(2,2), kernel_dims = 3):
	"""Creates the expansive path blocks used in the UNET

	Keyword arguments:
	c_large -- Output of the previous block.
	c_small -- Output of the corresponding contracting path block.
	num_filters -- The number of filters to be used in this block's layers.
	strides_dims -- The stride size to be used in this block's deconvolution layers (default 2x2).
	kernel_dims -- The kernel dimension used during this block's layers (default 3). 

	Returns:
	u -- The processed tensor after passing through this block's layers.
	"""
...
def get_unet(inp, num_classes=3, num_down_blocks=5, num_up_blocks=4):
	"""Creates a UNET model used in image segmentation. 
	Reference - https://arxiv.org/abs/1505.04597

	Keyword arguments:
	inp --  Input tensor.
	num_classes -- The number of classes (default 3)
	num_down_blocks -- Number of convolution block in the Unet (default 5)
	num_up_block -- Number of deconvolution blocks in the Unet (default 4)

	Returns:
	model -- The created model object, as per specifictions.
	"""
